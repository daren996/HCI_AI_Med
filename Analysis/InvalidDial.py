...
# get validity xxx cross dict - xxx : span, time, age, gender
...
def plot_cross_valid(valid_group_arr, cross_group, cross_type, title, x_label):
    ...
    ax1.set_title(title)
    plt.xticks(np.arange(len(vg_arr[cross_group[0]])) + bar_width * (len(valid_group_arr) - 1) / 2, labels,
               fontproperties=font)
...
vc_group = {'time': Utils.time_group, 'span': Utils.span_group, 'age': Utils.age_group, 'gender': Utils.gender_group}


if __name__ == '__main__':
    cross_type = 'span'  # time, span, age, gender
    valid_cross_group_arr = cross_anl_valid(vc_group[cross_type], source_path + data, cross_type=cross_type)
    print(valid_cross_group_arr)
    plot_cross_valid(valid_cross_group_arr, vc_group[cross_type], cross_type=cross_type,
